...
public class ComputeMatrix {
    ...
    public void compute() throws FileNotFoundException {
        ...
        for (int elementNumber = 0; elementNumber < globalData.getNe(); elementNumber++) { // ptla 16 po wszystkich elementach
            ...
            for (int intergrationPoint = 0; intergrationPoint < 4; intergrationPoint++) {
                ...
                // Na podstawie podrecznika
                detj = Math.abs(jakobian.getJakobianDet());
                ...
                for (int i = 0; i < 4; i++) {// 4 pkt
                    for (int j = 0; j < 4; j++) {
                        ...
                        valueFromMatrixC = globalData.getC() * globalData.getRo() * LocalElement.getN()[intergrationPoint][i] * LocalElement.getN()[intergrationPoint][j] * detj;

                        // wzor na macierz H
                        actual_H[i][j] += globalData.getK() * (dNdx[i] * dNdx[j] + dNdy[i] * dNdy[j]) * detj + valueFromMatrixC / globalData.getDtau();
                ...
                for (int p = 0; p < 2; p++) {
                    for (int n = 0; n < 4; n++) {
                        for (int i = 0; i < 4; i++) {
                            ...
                            actual_H[n][i] += globalData.getAlfa() * LocalElement.getSurfaceLocal()[elementID].shapeFunction[p][n] * LocalElement.getSurfaceLocal()[elementID].shapeFunction[p][i] * detj;
                        }
                        // W tym wzorze nie ma minusa ponieważ w równaniu 6.8 mamy minus
                        actual_P[n] += globalData.getAlfa() * globalData.gettEnvironment() * LocalElement.getSurfaceLocal()[elementID].shapeFunction[p][n] * detj;
                    }
                }
            }
